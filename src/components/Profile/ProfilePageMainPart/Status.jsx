...
import { fetchUiSpin, statusErrorText } from '../../../common/commonData'
...
const Status = ({ statusProps }) => {
     ...
     return (
          <div className='status-wrapper'>
               <div hidden={!statusLengthError || !statusEditMode} style={{ 'color': 'red' }}>{statusErrorText}</div>
               <div className={`status-container ${statusDataUploadStatus && 'status-container-data-uploaded'}`}>
                    {fetchStatusData ? <span>{fetchUiSpin}</span> : statusEditMode && !fetchStatusData ?
